...
# 3 different ways to calculate r squared:

# Regression sum of squares / total sum of squares:

anova.table <- anova(model)
ss.reg <- anova.table$`Sum Sq`[1]
ss.total <- anova.table$`Sum Sq`[1] + anova.table$`Sum Sq`[2]
ss.reg / ss.total

# [1] 0.488754

# Squaring the correlation coefficient (r):
cor(df$Number.of.meals.with.fish, df$Total.Mercury.in.mg.g)^2
# [1] 0.488754


# Built-in anova test function
anova.table <- summary(model)
anova.table$r.squared
# [1] 0.488754
...
# Construct a 90% confidence interal:

alpha <- 0.05
t <- qt((1-alpha/2), df=98)
t
SE.b.1 <- anova.table$coefficients[4]
b.1 <-  anova.table$coefficients[2]
...
# Using the t-distribution, we are 90% confident that the true value of Beta.1 is between 0.2275092 and 0.3243913


# Alternatively, we can conduct the same test, using R's builtin confint function:
...
r <- cov(df$Total.Mercury.in.mg.g, df$Number.of.meals.with.fish)
x.sd <- sd(df$Number.of.meals.with.fish)
y.sd <- sd(df$Total.Mercury.in.mg.g)
y.sd   #2.528044
x.sd   #6.404702

r <- cor(df$Total.Mercury.in.mg.g, df$Number.of.meals.with.fish)
r  
